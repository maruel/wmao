...
- `internal/server/dto/routes.go`: API route declarations used by the code generator to produce typed TS and Kotlin clients.
...
- `internal/server/eventconv.go`: Conversion from internal agent.Message types to dto.ClaudeEventMessage for
- `internal/server/genericconv.go`: Backend-neutral conversion from agent.Message to dto.EventMessage for SSE.
...
- `internal/server/titlegen.go`: Title generation using a cheap LLM call to summarize task conversations.
